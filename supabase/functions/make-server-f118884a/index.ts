...
import { MCP_TOOLS, handleMCPTool } from './mcp_server.ts';

const app = new Hono();
const FUNCTION_BASE = '/make-server-f118884a';
...
Deno.serve((req) => {
  ...
  // Supabase routes requests to the function root without including the
  // function name in the pathname. The existing API handlers were authored
  // with the function name prefixed, so we normalize requests here to keep
  // the frontend contract unchanged.
  if (!url.pathname.startsWith(FUNCTION_BASE)) {
    url.pathname = `${FUNCTION_BASE}${url.pathname}`;
  }
